...
class TicketLogController extends Controller
{
    public function index(Request $request)
    {
        ...
        $needApprovalTickets = Tickets::with(['sprintDetails.projectDetails', 'estimationApproval', 'workLogs.user'])
        ...
        foreach ($needApprovalTickets as $ticket) {
            ...
            $ticket->spent_hours = $ticket->workLogs->sum('hours');
            $ticket->remaining_hours = max((float)$ticket->time_estimation - $ticket->spent_hours, 0);
        ...
        $approvedNotStartedTickets = Tickets::with(['sprintDetails.projectDetails', 'estimationApproval', 'workLogs.user'])
            ->whereNotNull('time_estimation')
        ...
        $inProgressTickets = Tickets::with(['sprintDetails.projectDetails', 'estimationApproval', 'workLogs.user'])
            ->whereNotNull('time_estimation')
            ->whereHas('estimationApproval')
        ...
        $invoiceDoneTickets = Tickets::with(['sprintDetails.projectDetails', 'estimationApproval', 'workLogs.user'])
